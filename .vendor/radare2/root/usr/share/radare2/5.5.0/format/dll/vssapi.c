// SDB-CGEN V1.8.3
// gcc -DMAIN=1 vssapi.c ; ./a.out > vssapi.h
#include <ctype.h>
#include <stdio.h>
#include <string.h>
...
int gperf_vssapi_foreach(GperfForeachCallback cb, void *user) {
  int i = 0; while (kvs[i].name) {
  cb (user, kvs[i].name, kvs[i].value);
  i++;}
  return 0;
}
const char *gperf_vssapi_get(const char *s) {
  int i = 0; while (kvs[i].name) {
  if (!strcmp (s, kvs[i].name)) return kvs[i].value;
  i++;}
  return NULL;
}
#define sdb_hash_c_vssapi(x,y) gperf_vssapi_hash(x)
const unsigned int gperf_vssapi_hash(const char *s) {
  int sum = strlen (s);
  while (*s) { sum += *s; s++; }
  return sum;
}
struct {const char *name;void *get;void *hash;void *foreach;} gperf_vssapi = {
  .name = "vssapi",
  .get = &gperf_vssapi_get,
  .hash = &gperf_vssapi_hash,
  .foreach = &gperf_vssapi_foreach
};

#if MAIN
int main () {
	const char *s = ((char*(*)(char*))gperf_vssapi.get)("foo");
	printf ("%s\n", s);
}
#endif
